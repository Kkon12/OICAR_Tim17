...
export function QueueListScreen() {
  ...
  async function takeTicket(queue: QueueItem) {
    ...
    try {
      const ticket = await api.takeTicket(queue.id);
    ...
    <Screen>
      ...
      {selectedQueue && (
        <View style={styles.takeNumberPanel}>
          ...
          <SqButton
            title={
              loadingId === selectedQueue.id ? "Kreiram broj..." : "UZMI BROJ"
            }
            ...
            >
              ...
              <View style={styles.metaRow}>
                <Text
                  style={[
                    styles.wait,
                    queue.waitingCount === 0 && { color: colors.success },
                  ]}
                >
                  {queue.waitingCount === 0
                    ? "✓ Nema čekanja"
                    : `☷ ${queue.waitingCount} čeka`}
                </Text>
              </View>
              <Text
                style={[
                  styles.counters,
                  queue.openCounters === 0 && { color: colors.muted },
                ]}
              >
                {queue.openCounters > 0
                  ? `▣ otvorenih šaltera: ${queue.openCounters}`
                  : "Zatvoreno"}
              </Text>
              <View
                style={[
                  styles.badge,
                  queue.isActive ? styles.active : styles.closed,
                ]}
              >
                <Text style={styles.badgeText}>
                  {queue.isActive ? "Aktivan" : "Zatvoren"}
                </Text>
              </View>
